...
import { VoiceChannelOptions } from 'rainlink';
import { RainlinkWebsocket } from 'rainlink';
import { RainlinkDatabase } from 'rainlink';
...
export class RainlinkPlugin extends Plugin {
	...
	protected wsMessageEvent(node: RainlinkNode, data: string) {
		...
		case 'startSpeakingEvent': {
			this.manager?.emit("voiceStartSpeaking", node, wsData.data.userId, wsData.data.guildId);
			break;
		}
